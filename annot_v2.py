...
base_path = "E:/diwan_jersey/v2/bboxes"

legib_file_path = f"data\diwan_v2\diwan_v2_with_legib.json"
crops_with_legib = read_json(legib_file_path)
...
for crop_ctr, crop in enumerate(crops_with_legib):
    if crop["legib_prob"]<0.0:
        print(f"""crop: {crop_ctr+1}/{len(crops_with_legib)} | Probably illigible with  prob: {round(crop["legib_prob"],4)} """)
        continue
    ...
    text2 = f"unc: {crop['legib_prob']:.2f}"
    font = cv2.FONT_HERSHEY_SIMPLEX
    font_scale = 0.4
    thickness = 1
    color = (255, 0, 0)
    ...
    while True:
        ...
        if key == 27:  # ESC
            ...
            print(f"crop: {crop_ctr+1}/{len(crops_with_legib)} || {relative_path.split('/')[-1]} || marked as -1 (No number)")

        elif key == ord('w'):  # Unknown number
            annotations.append({'relative_path': relative_path, 'class': -2})
            print(f"crop: {crop_ctr+1}/{len(crops_with_legib)} || {relative_path.split('/')[-1]} || marked as -2 (Unknown number)")

        elif key == 13 :  # Confirmer la prédiction du modèle
            annotations.append({'relative_path': relative_path, 'class': predicted_class})
            print(f"crop: {crop_ctr+1}/{len(crops_with_legib)} || {relative_path.split('/')[-1]} || accepted model prediction: {predicted_class}")
            ...
            print(f"crop: {crop_ctr+1}/{len(crops_with_legib)} || {relative_path.split('/')[-1]} || manually entered: {number}")
